...
echo 'alias vin="$HOME/core-bridgechain/packages/core/bin/run"' >> ~/.bashrc

rm -rf "$HOME/core-bridgechain"
git clone "https://github.com/munich/10.git" "$HOME/core-bridgechain" || FAILED="Y"
if [ "$FAILED" == "Y" ]; then
    echo "Failed to fetch core repo with origin 'https://github.com/munich/10.git'"
...
rm -rf "$HOME/.config/@vin"
rm -rf "$HOME/.config/@vinichain"
rm -rf "$HOME/.config/vin-core"
